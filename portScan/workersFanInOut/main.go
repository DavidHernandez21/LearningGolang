...
func gen(done <-chan struct{}, ports ...int) <-chan scanOp {
	...
	go func() {
		...
		for _, p := range ports {
			...
			case <-done:
				fmt.Println("recieved mex from done channel gen function")
				return
			}
		}
	}()
	...
	go func() {
		...
		for scan := range in {
			...
			default:
				...
				out <- scan
			case <-done:
				fmt.Println("recieved mex from done channel scan function")
				return
			}
		}
	}()
...
func filterOpen(done <-chan struct{}, in <-chan scanOp) <-chan scanOp {
	...
	go func() {
		...
		for scan := range in {
			...
			case <-done:
				fmt.Println("recieved mex from done channel filterOpen function")
				return
			}
		}
	}()
...
func filterErr(done <-chan struct{}, in <-chan scanOp) <-chan scanOp {
	...
	go func() {
		...
		for scan := range in {
			...
			case <-done:
				fmt.Println("recieved mex from done channel filterErr function")
				return
			}
		}
	}()
	...
	for _, sc := range chans {
		go func(sc <-chan scanOp) {
			...
			for scan := range sc {
				...
				case out <- scan:
				case <-done:
					fmt.Println("recieved mex from done channel merge function")
					return
				}
			}
		}(sc)
