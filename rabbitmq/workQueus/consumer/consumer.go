...
func consumeMexwithtimeout(chDelivery <-chan amqp.Delivery, t time.Duration) {
	...
	for {
		...
		case delivery := <-chDelivery:
			log.Println(fmt.Sprintf("msg: %s - %d - %d - %s - %s - %s", string(delivery.Body), delivery.DeliveryMode,
...
func main() {
	...
	go func() {
		for delivery := range chDelivery {
			log.Println(fmt.Sprintf("msg: %s - %d - %d - %s - %s - %s - %t - %v", string(delivery.Body), delivery.DeliveryMode,
